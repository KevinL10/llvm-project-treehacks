...
The LLVM backend should be developed in llvm/lib/Target/Fits. To test the assembler, run:

```sh
cd ~/Downloads/liveshare && ./build_full.sh demo/main.c
```

Then read `code.s` to see the assembly.
...
The col is in bytes, not 32-bit words, but must be word-aligned.
